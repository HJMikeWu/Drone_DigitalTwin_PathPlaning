...
class PSOWarehouseRealObstacles:
    ...
    def place_drone_at_current_start(self):
        """Place the existing drone actor at the current start position."""
        if not hasattr(self, "drone"):
            return
        if not hasattr(self.drone, "set_world_pose"):
            return
    ...
    def save_convergence_curve(self, fitness_history, simulation_count, timestamp):
        """Save PSO fitness convergence data to CSV and generate a PNG plot."""
        ...
        csv_out = os.path.join(movies_dir, f"pso_fitness_round_{int(simulation_count):03d}_{timestamp}.csv")
        ...
        try:
            ...
            ax.plot(iterations, fitnesses, linewidth=1.5, color="royalblue")
            ax.set_xlabel("PSO Iteration")
            ax.set_ylabel("Best Fitness")
            ax.set_title(f"PSO Fitness Convergence — Round {simulation_count}")
        ...
        while simulation_app.is_running():
            ...
            try:
                ...
                while has_collision and retry_count < MAX_RETRY:
                    retry_count += 1
                    print(f"\n[RETRY] Collision detected on current path: {col_segs} colliding segment(s).")
                    print(f"[RETRY] Re-running PSO optimization ({retry_count}/{MAX_RETRY}) to find a collision-free path...")
                ...
                self.world.reset()
                self.place_drone_at_current_start()
                self.timeline.play()
                ...
                if collision_occurred:
                    ...
                    if reached_goal:
                        ...
                        while True:
                            try:
                                rerun_choice = input("Re-run simulation with new start/goal positions? [Y/N]: ").strip().lower()
                            except EOFError:
                                rerun_choice = "n"
