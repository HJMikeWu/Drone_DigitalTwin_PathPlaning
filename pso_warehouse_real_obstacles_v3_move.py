...
# 這會確保所有擴充功能（如 Pegasus 和 Debug Draw）正確載入
...
while time.time() - start_time < 60:  # 最多等待60秒
    ...
    try:
        if hasattr(simulation_app, 'is_app_ready') and simulation_app.is_app_ready():
            print("Isaac Sim 應用程式已準備就緒！")
            break
        elif hasattr(simulation_app, 'is_running') and simulation_app.is_running():
            print("Isaac Sim 應用程式正在運行...")
            break
    except:
        pass
    time.sleep(0.5)

print("Isaac Sim 啟動階段完成，開始執行我們的代碼...")
...
class PSOWarehouseRealObstacles:
    """
    PSO 路徑規劃模擬 - 使用真實場景障礙物偵測 (v3 版本)
    
    核心特色：
    - 從 USD Stage 自動遍歷所有幾何體
    - 透過 Bounding Box 計算自動識別障礙物
    - 開啟時自動掃描可用空間
    - 根據空間範圍隨機生成不碰撞障礙物的起點與終點
    """
        ...
        while simulation_app.is_running():
            ...
            try:
                ...
                if collision_occurred:
                    ...
                    if reached_goal:
                        print("本輪已完成到點並存檔，保留視窗供查看，手動關閉視窗後才會結束程式。")
                        while simulation_app.is_running():
                            self.visualize_pso_step()
                            self.update_follow_camera_view()
                            simulation_app.update()
